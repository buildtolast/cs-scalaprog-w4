...
object Huffman {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encodeEach(tree: CodeTree)(t: Char): List[Bit] = {
      tree match {
        case l: Leaf => Nil
        case f: Fork => {
          if (f.chars.contains(t)) {
            f.left match {
              case l1: Leaf => if (l1.char == t) bits = bits ::: List(0) else Nil
              case f1: Fork => if (f1.chars.contains(t)) {
                bits = bits ::: 0 :: encodeEach(f1)(t)
                bits
              } else Nil
            }
            f.right match {
              case l2: Leaf => if (l2.char == t) bits = bits ::: List(1) else Nil
              case f2: Fork => if (f2.chars.contains(t)) {
                bits = bits ::: 1 :: encodeEach(f2)(t)
                bits
              } else Nil
            }
          } else Nil
    ...
    var r : List[Bit] = Nil
    if (text.nonEmpty) {
      bits = bits ::: encodeEach(tree)(text.head) ::: encode(tree)(text.tail)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    val map = table.map(b => b._1 -> b._2).toMap
    map(char)
  }
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    case l : Leaf => List((l.char, encode(createCodeTree(List(l.char)))(List(l.char))))
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    val m = b.map(e => e._1 -> e._2).toMap
    a.foreach(e => if(m.contains(e._1)) m(e._1) :: e._2 else m + (e._1 -> e._2))
    m.toStream.groupBy(e => (e._1, e._2)).keys.toList
  }
